...
namespace CreateDeviceIdentity
{
    class Program
    {
        ...
        private static async Task UpdateDeviceTwin(string deviceId, string lat, string lon)
        {
            try
            {
                var twin = await registryManager.GetTwinAsync(deviceId);
                var patch = @"{tags:{location:{latitude: '" + lat + "',longitute: ' " + lon + "'}}}";
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < deviceName.Length; i++)
            {
                ...
                UpdateDeviceTwin(deviceName[i], latitude[i], longitude[i]).Wait();
            }
            Console.ReadLine();
        }
    }
