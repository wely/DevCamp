...
namespace DevCamp.WebApp.Controllers
{
    public class IncidentController : Controller
    {
        ...
        private static EmailMessage getEmailBodyContent(Incident incidentData, string EmailFromAddress)
        {
            ...
            EmailAddress emailTo = new EmailAddress() { name = EmailFromAddress, address = EmailFromAddress };
		...
		private async Task CreateEvent(Incident incidentData)
		{
			string userObjId = ClaimsPrincipal.Current.FindFirst(Settings.AAD_OBJECTID_CLAIMTYPE).Value;
			SessionTokenCache tokenCache = new SessionTokenCache(userObjId, HttpContext);
			string tenantId = ClaimsPrincipal.Current.FindFirst(Settings.AAD_TENANTID_CLAIMTYPE).Value;
			...
			using (var client = new HttpClient())
			{
				...
				if (response.IsSuccessStatusCode)
				{
					...
					contacts = JsonConvert.DeserializeObject<ContactValue>(resultString);
				}
			}

			Event evnt = getEvent(incidentData, contacts);
			...
			using (var client = new HttpClient())
			{
				...
				StringContent msgContent = new StringContent(JsonConvert.SerializeObject(evnt), System.Text.Encoding.UTF8, "application/json");
				msgContent.Headers.ContentType = new MediaTypeWithQualityHeaderValue("application/json");
				HttpResponseMessage response = await client.PostAsync(Settings.GRAPH_EVENTS_URL, msgContent);
				if (response.IsSuccessStatusCode)
				{
					string resultString = await response.Content.ReadAsStringAsync();
				}
		...
		private Event getEvent(Incident incidentData, ContactValue contacts)
		{
			Event evnt = new Event();
			evnt.location.displayName = incidentData.City;
			evnt.body.content = incidentData.Description;
			evnt.body.contentType = Settings.EMAIL_MESSAGE_TYPE;
			evnt.subject = incidentData.OutageType;
			evnt.attendees = new Attendee[]
			{
				new Attendee() { emailAddress = contacts.value[0].emailAddresses[0], type = "required" },
				new Attendee() { emailAddress = contacts.value[1].emailAddresses[0], type = "required" },
				new Attendee() { emailAddress = contacts.value[2].emailAddresses[0], type = "required" },
			};
			evnt.start.timeZone = "Pacific Standard Time";
			DateTime startTime = GetEventTime();
			evnt.start.dateTime = $"{startTime.Year}-{startTime.Month}-{startTime.Day}T{startTime.Hour}:00:00";
			evnt.end.timeZone = "Pacific Standard Time";
			evnt.end.dateTime = $"{startTime.Year}-{startTime.Month}-{startTime.Day}T{startTime.Hour + 1}:00:00";
			return evnt;
		}
